function createMCSimulation(box) {
    ...
    function VDW(dr, eps, sig) {
        ...
        return {
            en: -eps * s6,          // Corrected to -1 * eps * s^6
            xi: -0.25 * eps * s6    // Scaled by 1/24th to match the pcoef 8-factor trick
        };
    }
    ...
    // Numerical Integrator for VDW Second Virial Coefficient
    function computeB2_VDW(eps, sig, T) {
        const rMin = sig; // Integration of the tail starts exactly at sigma
        const rMax = 10.0 * sig; 
        const nSteps = 1000;
        const dr = (rMax - rMin) / nSteps;
        ...
        for (let i = 0; i <= nSteps; i++) {
            ...
            const s = sig / r;
            const s6 = Math.pow(s, 6);
            const v = -eps * s6; // Updated to match the strict -1 * eps * s^6 definition
        ...
        const coreIntegral = -Math.pow(sig, 3) / 3.0; // Hard-sphere analytical core
        return -2.0 * Math.PI * (coreIntegral + integral);
    }
        ...
        if (s.species.type === "IG") {
            ...
            if (s.species.type === "LJ") {
                ...
                if (s.computeGr && s.grSamples > 0) {
                    ...
                    // Adjusted the 16 factor to 4 to match the updated potential definition
                    const Z_tail = - (4 * Math.PI * rho * eps * Math.pow(sig, 6) / s.T) * tailIntegral;
    ...
    function finalize(s) {
        ...
        if (s.species.type === "LJ" || s.species.type === "VDW") {
            ...
            if (s.species.type === "LJ") {
                ...
                // Adjusted LRC factors to match the correct potential scale (8 -> 2, 16 -> 4)
                e_lrc = -s.N * (2.0 / 3.0) * Math.PI * rho * s.species.eps * sig3 * s_rc3;
                p_lrc = -(4.0 / 3.0) * Math.PI * rho * rho * s.species.eps * kB * sig3 * s_rc3;
            }
        ...
        if (["HS", "SW", "VDW", "LJ"].includes(s.species.type)) {
            ...
            if (s.species.type === "LJ") {
                ...
                // Now uses numerical integration
