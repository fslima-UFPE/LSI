...
subtitle: A entropia
description: "Aula da segunda lei da termodinâmica e da entropia. Definição da entropia de Carathéodory e de Carnot. Definição química da segunda lei da termodinâmica."
...
<p class="jt">
  Para uma função de estado, teremos que: $\oint dU = 0  $   ou   $  \sum_i \Delta_i U = 0$
</p>
...
<p class="jt"><b>3.</b> Caminho <span style="color: orange;">D</span>+<span style="color: blue;">E</span>, composto por (D) um aquecimento isobárico reversível de P<sub>1</sub> até P<sub>4</sub> e, (E) um resfriamento isovolumétrico reversível de P<sub>4</sub> até P<sub>2</sub>.
</p>
...
<p class="jt">
  A Figura 1 mostra três caminhos diferentes para a transformação referida. Como os pontos inicial e final possuem a mesma temperatura, e sendo o gás é ideal, então $\boldsymbol{\Delta_A U = \Delta_{B+C} U = \Delta_{D+E} U = 0}$, uma vez que a energia interna é função de caminho e depende apenas da temperatura para um G.I., como visto na equação <a href="../A7/#TC7.11" style="color: blue; font-style: italic;">TC7.11c</a>. Mas, sendo todos os caminhos reversíveis, seus calores serão diferentes mesmo assim? A resposta para essa pergunta é <b>sim</b> e podemos calcular o calor de cada caminho através de:
</p>

<p class="jt">
  Caminho <span style="color: black;">A</span>:  $\boldsymbol{q_{rev,A}} = -w_{rev,A} = \boldsymbol{nRT ln \left( \frac{V_2}{V_1} \right)}$ <br>
  Caminho <span style="color: red;">B</span>+<span style="color: green;">C</span>:  $\boldsymbol{q_{rev,B+C}} = q_{rev,B} + q_{rev,C} = 0 + \int_{T_2}^{T_1} C_V dT = \boldsymbol{\int_{T_2}^{T_1} C_V dT}$ <br>
  Caminho <span style="color: orange;">D</span>+<span style="color: blue;">E</span>:  $\boldsymbol{q_{rev,D+E}} = q_{rev,D} + q_{rev,E} = \Delta U_D - w_{rev,D} + \int_{T_3}^{T_1} C_V dT \\ = {}$<br>
$\boldsymbol{\int_{T_1}^{T_3} C_V dT + p_1 (V_2 - V_1) + \int_{T_3}^{T_1} C_V dT = p_1 (V_2 - V_1)}$ <br>
</p>
...
$\frac{Q_f}{Q_q} = \frac{nRT_f ln \left( {V_4}/{V_3}\right)}{nRT_q ln \left( {V_2}/{V_1}\right)} = \frac{T_f}{T_q} \frac{ln \left( {V_4}/{V_3}\right)}{ln \left( {V_2}/{V_1}\right)}$
</p>
...
<div id="TC8.3" class="mathbox-wrapper">
  <div class="mathbox">
    $$ \color{blue}{\boldsymbol{dS = \lambda(T) đq_{rev}}}$$
  </div>
  <div class="eq-numbers">
...
<p class="jt">
  A equação <a href="#TC8.3" style="color: blue; font-style: italic;">TC8.3</a> define que a entropia será o diferencial do calor reversível multiplicado por um fator de integração, que Carathéodory definiu como uma função de T, apenas. É possível demonstrar que essa função será: $\lambda(T) = \frac{c}{T}$, sendo $c$ uma constante a ser definida. Daí, é possível definir o diferencial da entropia como:
</p>

<div id="TC8.4" class="mathbox-wrapper">
  <div class="mathbox">
    $$ \color{blue}{\boldsymbol{dS = c \frac{đq_{rev}}{T}}}$$
  </div>
  <div class="eq-numbers">
...
<p class="jt">
  O próximo passo será considerar a entropia uma propriedade extensiva. Logo, ela também é aditiva e podemos dizer que a variaçãod entropia total será a soma das variações de entropia dos subsistemas A e B. Logo, $dS = dS_A + dS_B = c \frac{đq_{rev,A}}{T_A} + c \frac{đq_{rev,B}}{T_B}$. Como as variações de energia interna dos dois subsistemas não são independentes, podemos reescrever essa equação na forma:
</p>

<p class="jt" style="font-size: 1.3em;">
   $dS = c \left(\frac{1}{T_A} - \frac{1}{T_B} \right) dU_A $
</p>
