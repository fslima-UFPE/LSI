...
<b>Figura 1.</b> Um ciclo levando dos reagentes aos produtos por dois caminhos: diretamente, ou passando para as substâncias elementares que, em seguida, formarão os produtos.
</p>
...
<div id="EC12.9" class="chembox-wrapper">
  <div class="chembox">
    $$\begin{gather}    
    2CH_3 OH(g) + NH_3(g) \to (CH_3)_2 NH(g) + 2H_2 O(g)
    \end{gather}$$    
  </div>
  <div class="eq-numbers">    
    <span style="font-weight: bold;">EC12.9</span>
  </div>
</div>

<p class="jt">
  Vamos escrever abaixo as equações termoquímicas padrão de formação de cada uma das substâncias envolvidas na reação ($CH_3OH(g)$, $NH_3(g)$, $(CH_3)_2NH(g)$ e $H_2O$), a 298 K:
</p>

<div id="EC12.10" class="chembox-wrapper">
  <div class="chembox">
    $$\begin{flalign*}    
    && \color{red}{C(grafite) + 2H_2(g) + \tfrac{1}{2}O_2(g) \to CH_3OH(g)} \qquad \Delta_1 H^o = -200,7 \ kJ.mol^{-1}\\
    && \color{green}{\tfrac{1}{2}N_2(g) + \tfrac{3}{2}H_2(g) \to NH_3(g)} \qquad \Delta_2 H^o = -45,9 \ kJ.mol^{-1}\\
    && \color{orange}{2C(grafite) + \tfrac{7}{2}H_2(g) + \tfrac{1}{2}N_2(g) \to (CH_3)_2 NH(g)} \qquad \Delta_3 H^o = -18,6 \ kJ.mol^{-1}\\
    && \color{blue}{H_2(g) + \tfrac{1}{2}O_2(g) \to H_2O(g)} \qquad \Delta_4 H^o = -241,8 \ kJ.mol^{-1}
    \end{flalign*}$$    
  </div>
  <div class="eq-numbers">    
    <span style="font-weight: bold;">EC12.10</span>
  </div>
</div>

<p class="jt">
  Podemos imaginar essa reação ocorrendo em duas partes. A primeira é a reação de <b>decomposição</b> (padrão) dos reagentes, isto é, a reação inversa da reação de formação:
</p>

<p class="jt">
<span style="color: red;">1)</span> Dois mols de metanol se decompondo em quantidades estequiométrias de $C(grafite)$, $H_2(g)$ e $O_2(g)$; <br>
<span style="color: green;">2)</span> Um mol de amônia se decompondo em quantidades estequiométricas de $N_2(g)$ e $H_2(g)$.
</p>

<p class="jt">
  E a soma dessas duas equações químicas é:
</p>

<div id="EC12.11" class="chembox-wrapper">
  <div class="chembox">
    $$\begin{flalign*}    
    && 2 \times [{\color{red}{CH_3OH(g) \to C(grafite) + 2H_2(g) + \tfrac{1}{2}O_2(g)}}] \qquad \Delta_a H^o = -2 \times (-200,7 \ kJ.mol^{-1})\\
    && 1 \times [{\color{green}{NH_3(g) \to \tfrac{1}{2}N_2(g) + \tfrac{3}{2}H_2(g)}}] \qquad \Delta_b H^o = -1 \times (-45,9 \ kJ.mol^{-1})\\
    \end{flalign*}$$
    <hr style="border: 0; border-top: 1.5px solid #666; margin: 4px 0;">
    $$2CH_3(g) + NH_3(g) \to 2C(grafite) + \tfrac{11}{2}H_2(g) + O_2(g) + \tfrac{1}{2}N_2(g) \qquad \Delta_c H^o = +447,3 \ kJ.mol^{-1}$$
  </div>
  <div class="eq-numbers">    
    <span style="font-weight: bold;">EC12.11</span>
  </div>
</div>

<p class="jt">
  Agora, as substâncias simples formadas pela decomposição dos reagentes podem se recombinar para formar os produtos desejados através de <b>reações-padrão de formação</b>:
</p>

<p class="jt">
<span style="color: orange;">3)</span> Um mol de dietilamina é formado a partir de quantidades estequiométricas de $C(grafite)$, $H_2(g)$ e $N_2(g)$; <br>
<span style="color: blue;">3)</span> Dois mols de água são formados a partir de quantidades estequiométricas de $H_2(g)$ e $O_2(g)$. <br>
</p>
...
<div id="EC12.12" class="chembox-wrapper">
  <div class="chembox">
    $$\begin{flalign*}    
    && 1 \times [{\color{orange}{2C(grafite) + \tfrac{7}{2}H_2(g) + \tfrac{1}{2}N_2(g) \to (CH_3)_2 NH(g)}}] \qquad \Delta_x H^o = 1 \times (-18,6 \ kJ.mol^{-1})\\
    && 2 \times [{\color{blue}{H_2(g) + \tfrac{1}{2}O_2(g) \to H_2O(g)}}] \qquad \Delta_y H^o = 2 \times (-241,7 \ kJ.mol^{-1})\\
    \end{flalign*}$$
    <hr style="border: 0; border-top: 1.5px solid #666; margin: 4px 0;">
    $$2C(grafite) + \tfrac{11}{2}H_2(g) + O_2(g) + \tfrac{1}{2}N_2(g) \to (CH_3)_2NH(g) + 2 H_2O(g) \qquad \Delta_z H^o = -502,0 \ kJ.mol^{-1}$$
  </div>
  <div class="eq-numbers">    
    <span style="font-weight: bold;">EC12.12</span>
  </div>
</div>

<p class="jt">
  Finalmente, podemos somar as duas partes (<a href="#EC12.11" style="font-weight: bold; color: black;">EC12.11</a> e <a href="#EC12.12" style="font-weight: bold; color: black;">EC12.12</a>) para obter a reação desejada (<a href="#EC12.9" style="font-weight: bold; color: black;">EC12.9</a>):
</p>

<div id="EC12.13" class="chembox-wrapper">
  <div class="chembox">
    $$\begin{flalign*}    
    && 2CH_3(g) + NH_3(g) \to \cancel{2C(grafite)} + \cancel[green]{\tfrac{11}{2}H_2(g)} + \cancel[orange]{O_2(g)} + \cancel[blue]{\tfrac{1}{2}N_2(g)} \qquad \Delta_c H^o = +447,3 \ kJ.mol^{-1}\\
    && \cancel{2C(grafite)} + \cancel[green]{\tfrac{11}{2}H_2(g)} + \cancel[orange]{O_2(g)} + \cancel[blue]{\tfrac{1}{2}N_2(g)} \to (CH_3)_2NH(g) + 2 H_2O(g) \qquad \Delta_z H^o = -502,0 \ kJ.mol^{-1}
    \end{flalign*}$$
    <hr style="border: 0; border-top: 1.5px solid #666; margin: 4px 0;">
    $$2CH_3 OH(g) + NH_3(g) \to (CH_3)_2 NH(g) + 2H_2 O(g) \qquad \Delta_r H^o = -54,7 \ kJ.mol^{-1}$$
  </div>
  <div class="eq-numbers">    
    <span style="font-weight: bold;">EC12.13</span>
  </div>
</div>

<p class="jt">
  E a entalpia dessa reação? É fácil ver que ela é igual a:<br>
  <br>
  $\Delta_r H^o = 1 \times \Delta_f H^o[(CH_3)_2 NH, g] + 2 \times \Delta_f H^o[H_2O, g] - 2 \times \Delta_f H^o[CH_3 OH, g] - 1 \times \Delta_f H^o[NH_3, g]$<br>
  
  E podemos reescrever essa expressão em termos de $\Delta_f H^o(i,ef)$ e de $\nu_i$:
</p>

<div id="TC12.5" class="mathbox-wrapper">
  <div class="mathbox">
    $$\begin{gather}
    \color{blue}{\boldsymbol{\Delta_r H^o = \sum_i \nu_i \Delta_f H^o(i,ef)}}
    \end{gather}$$
  </div>
  <div class="eq-numbers">    
    <span style="font-weight: bold; color: blue;">TC12.5</span>
  </div>
</div>

<p class="jt">
  Mas o resultado acima é geral: é possível escrever toda e qualquer entalpia-padrão de reação a partir das entalpias-padrão de formação dos reagentes e dos produtos. Como essa regra é geral e vale para todos os tipos de substâncias (orgânicas, inorgânicas), então tabular o valor da entalpia-padrão de formação de substâncias é a única necessidade para construirmos um banco de dados completo e útil.
</p>

### Substâncias elementares nos estados físico e alotrópicos mais estáveis

<p class="jt">
  Uma das consequências da definição de uma reação-padrão de formação de uma substância é que substâncias elementares nos estados físico e alotrópico mais estáveis naquela temperatura possuirão entalpia-padrão de formação igual a zero. O motivo? Vamos considerar, por exemplo, o caso de oxigênio molecular, na forma de $O_2$, na fase gasosa. Caso quiséssemos definir a sua reação-padrão de formação, escreveríamos:
</p>


<div id="EC12.14" class="chembox-wrapper">
  <div class="chembox">
    $$\begin{gather}    
    O_2(g) \to O_2(g) \qquad \Delta H^o = 0    
    \end{gather}$$    
  </div>
  <div class="eq-numbers">    
    <span style="font-weight: bold;">EC12.14</span>
  </div>
</div>

<p class="jt">
isto é, a reação-padrão de formação de uma substância elementar nos seus estados físico e alotrópicos mais estáveis é a transformação da substância nela mesma! Logo, tal transformação possui uma entalpia associada igual a zero - não houve nenhum tipo de processo. Então, por definição, $\Delta_f H^o = 0$.
</p>

## A equação de Kirchhoff

<p class="jt">
  Por fim, vamos estabelecer a relação entre a variação de entalpia de uma reação com a variação de temperatura. Vamos integrar a equação <a href="../A10/#TC10.4" style="color: blue; font-style: italic;">TC10.4c</a> e combinar o seu resultado com a definição de entalpia de reação da equação <a href="#TC12.4" style="color: blue; font-style: italic;">TC12.4b</a>, em um processo com todas as substâncias a $p = 1 \ bar$:
</p>


<div id="EC12.15" class="chembox-wrapper">
  <div class="chembox">
    $$\begin{gather}    
    {\bar H}_i^o(T_2) = {\bar H}_i^o(T_1) + \int_{T_1}^{T_2} {\bar C}_{p,i}^o dT\\
    \Delta_r H^o(T_2) = \sum_i \nu_i [{\bar H}_i^o(T_1) + \int_{T_1}^{T_2} {\bar C}_{p,i}^o dT]\\
    \Delta_r H^o(T_2) = \sum_i \nu_i {\bar H}_i^o(T_1) + \int_{T_1}^{T_2} \sum_i \nu_i {\bar C}_{p,i}^o dT\\
    \color{blue}{\boldsymbol{\Delta_r H^o(T_2) = \Delta_r H^o(T_2) + \int_{T_1}^{T_2} \Delta_r {\bar C}_p^o dT}}
    \end{gather}$$    
  </div>
  <div class="eq-numbers">    
    <span style="font-weight: bold;">EC12.15</span>
  </div>
</div>

<p class="jt">
  A equação acima é chamada de <b>equação de Kirchhoff</b> e definimos na última linha a <i>variação de capacidade térmica da reação</i>, na forma: $\Delta_r {\bar C}_p^o = \sum_i \nu_i {\bar C}_{p,i}^o$. Esta equação permite relacionarmos a variação de entalpia de uma reação, a uma dada temperatura, com a variação de entalpia da mesma reação, em outra temperatura.
</p>

<p class="jt">
  Com a entalpia definida e discutida, podemos introduzir a última grande lei da termodinâmica - tópico da próxima aula.
</p>
