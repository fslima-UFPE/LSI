...
<p class="jt">
  De outra forma, a equação termoquímica acima pode ser toda sumarizada em apenas uma expressão: $\boldsymbol{\Delta_f H^o_{298}(C_2 H_5 OH,l) = -277,7 \ kJ.mol^{-1}}$. Chamamos o termo $\Delta_f H^o_T(i,ef)$ de entalpia-padrão de formação da substância $i$, em um estado físico $ef$, a uma tempeartura $T$. A vantagem desta definição? É a possibilidade de escrever toda e qualquer reação química a partir de um conjunto pequeno de outras reações: processos químicos sempre conservam a quantidade de átomos de todos os elementos envolvidos na reação. Assim, podemos imaginar um ciclo no qual cada reagente, nas quantidades necessárias, irá formar suas respectivas substâncias elementares. Estas, por sua vez, irão se recombinar para fomar os produtos, nas quantidades adequadas! Mesmo que o caminho efetivamente tomado não seja esse, para o cálculo da entalpia da reação não importa - e nem para a reação global.
</p>
