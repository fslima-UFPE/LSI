...
<p class="jt">
  A primeira reação (<a href="#EC12.5" style="font-weight: bold; color: black;">TC12.5a</a>) nos diz que um equivalente da substância $A$ reage com um equivalente da substância $D$, para formar um equivalente da substância $X$, com entalpia igual a $m \ kJ.mol^{-1}$. Já a segunda reação (<a href="#EC12.5" style="font-weight: bold; color: black;">TC12.5b</a>) nos diz que um equivalente de $B$ reage para formar um equivalente de $D$ e um equivalente de $Y$, com entalpia igual a $n \ kJ.mol^{-1}$. Vamos imaginar, agora, o seguinte processo com todas substâncias gasosa e no estado-padrão, a uma temperatura $T$:
</p>
  
<p class="jt">
<p style="font-color: red;">1)</p> Um recipiente contendo um equivalente da substância $B$, que irá se transformar em um equivalente de um a substância $D$ e um equivalente de $Y$; <br>
<p style="font-color: blue;">2)</p> Adicionamos a esse recipiente um equivalente da substância $A$, que irá reagir com um equivalente da substância $D$ para formar um equivalente da substância $X$.
</p>
...
<div id="EC12.6" class="chembox-wrapper">
  <div class="chembox">
    $$\begin{gather}    
    \boldsymbol{B(g) \to \cancel{D(g)} + Y(g)} \qquad \Delta_{2} H^o(T) = n \ kJ.mol^{-1}\\
    \boldsymbol{A(g) + \cancel{D(g)} \to X(g)} \qquad \Delta_{1} H^o(T) = m \ kJ.mol^{-1}
    \end{gather}$$
    <hr style="border: 0; border-top: 1.5px solid #666; margin: 4px 0;">
    $$\boldsymbol{A(g) + B(g) \to X(g) + Y(g) \qquad \Delta_r H^o(T)} = h \ kJ.mol^{-1} = (m+n) \ kJ.mol^{-1}$$
  </div>
  <div class="eq-numbers">    
...
<div id="EC12.7" class="chembox-wrapper">
  <div class="chembox">
    $$\begin{gather}    
    \boldsymbol{2 C(grafite) + 3 H_2(g) + \frac{1}{2} O_2 \to C_2 H_5 O H(l)}
    \end{gather}$$    
  </div>
  <div class="eq-numbers">    
...
<div id="EC12.8" class="chembox-wrapper">
  <div class="chembox">
    $$\begin{gather}    
    \boldsymbol{2 C(grafite) + 3 H_2 (g) + \frac{1}{2} O_2(g) \to H_2 O(l)} \qquad \Delta H^o = -277,7 \ kJ.mol^{-1}
    \end{gather}$$    
  </div>
  <div class="eq-numbers">    
...
<p class="jt">
  De outra forma, a equação termoquímica acima pode ser toda sumarizada em apenas uma expressão: $\boldsymbol{\Delta_f H^o_{298}(C_2 H_5 OH,l) = -277,7 \ kJ.mol^{-1}}$. Chamamos o termo $\Delta_f H^o_T(i,ef)$ de <b>entalpia-padrão de formação</b> da substância $i$, em um estado físico $ef$, a uma tempeartura $T$. A vantagem desta definição? É a possibilidade de escrever toda e qualquer reação química a partir de um conjunto pequeno de outras reações: processos químicos sempre conservam a quantidade de átomos de todos os elementos envolvidos na reação. Assim, podemos imaginar um ciclo no qual cada reagente, nas quantidades estequiométricas necessárias, irá formar suas respectivas substâncias elementares. Estas, por sua vez, irão se recombinar para fomar os produtos, também em quantidades estequiométricas! Mesmo que o caminho efetivamente tomado não seja esse, essa diferença não importa para o cálculo da entalpia da reação - e nem para a reação global, o que efetivamente foi consumido e produzido.
</p>
...
<p class="jt">
  Vamos analisar a reação-padrão em fase gasosa entre amônia e metanol, a 298 K, formando dietilamina e água:
</p>

<div id="EC12.9" class="chembox-wrapper">
  <div class="chembox">
    $$\begin{gather}    
    \boldsymbol{2CH_3 OH(g) + NH_3(g) \to (CH_3)_2 NH(g) + 2H_2 O(g)}
    \end{gather}$$    
  </div>
  <div class="eq-numbers">    
...
<p class="jt">
  Podemos imaginar essa reação ocorrendo em duas partes. A primeira é a <b>decomposição</b> dos reagentes, isto é, a reação inversa da reação de formação:
</p>
...
<div id="EC12.10" class="chembox-wrapper">
  <div class="chembox">
    $$\begin{gather}    
    2CH_3OH(g) \to 2C(grafite) + 4H_2(g) + O_2(g)\\
    NH_3(g) \to \frac{1}{2}N_2(g) + \frac{3}{2}H_2(g)\\
    \end{gather}$$
    <hr style="border: 0; border-top: 1.5px solid #666; margin: 4px 0;">
    $$\boldsymbol{2CH_3(g) + NH_3(g) \to 2C(grafite) + \frac{11}{2}H_2(g) + O_2(g) + \frac{1}{2}N_2(g)}$$
  </div>
  <div class="eq-numbers">    
...
<p class="jt">
  Agora, as substâncias simples formadas podem reagir entre si para formar os produtos desejados através de <b>reações-padrão de formação</b>:
</p>
...
<div id="EC12.11" class="chembox-wrapper">
  <div class="chembox">
    $$\begin{gather}    
    2C(grafite) + \frac{7}{2}H_2(g) + \frac{1}{2}N_2(g) \to (CH_3)_2 NH(g)\\
    2H_2(g) + O_2(g) \to 2H_2O(g)\\
    \end{gather}$$
    <hr style="border: 0; border-top: 1.5px solid #666; margin: 4px 0;">
    $$\boldsymbol{2C(grafite) + \frac{11}{2}H_2(g) + O_2(g) + \frac{1}{2}N_2(g) \to (CH_3)_2NH(g) + 2 H_2O(g)}$$
  </div>
  <div class="eq-numbers">    
