...
<script>
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    function createWrapperGroup(elementId) {
      ...
      // Fallback selector for waves if Inkscape used slightly different naming
    ...
    function animateSystem() {
      if (progress >= 1) {
        ...
        // Force waves to be completely invisible when stirring finishes
        if (waves.wrapper) waves.wrapper.style.opacity = '0';
        return;
      }

      progress += 0.0025;
      rotationAngle += 4.0; // Spin speed

      // 1. Rotate Blades around true visual center
      if (blades.wrapper) {
        blades.wrapper.setAttribute('transform', `rotate(${rotationAngle}, ${cx}, ${cy})`);
      }

      // 2. Rotate Waves around stirrer center + smooth sine curve fade (peaks mid-stir, 0 at end)
      if (waves.wrapper) {
        waves.wrapper.setAttribute('transform', `rotate(${rotationAngle}, ${cx}, ${cy})`);
        const waveOpacity = Math.sin(progress * Math.PI) * 0.85;
        waves.wrapper.style.opacity = waveOpacity.toString();
      }

      // 3. Ice Cubes wobble gently in place while melting
      if (ice.wrapper) {
        const wobbleX = Math.sin(rotationAngle * 0.035) * 0.5;
        const wobbleY = Math.cos(rotationAngle * 0.04) * 0.5;
