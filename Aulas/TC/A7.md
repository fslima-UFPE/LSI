...
<div class="simulation-box">
  <!-- Target container for the external SVG file -->
...
<script>
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    // 1. Asynchronously fetch external SVG graphic
    try {
      ...
      console.error('Error fetching external SVG:', error);
      startBtn.innerText = "Erro ao carregar SVG";
      return;
    }

    // Enable button after successful SVG load
    startBtn.innerText = "Realizar trabalho";
    startBtn.disabled = false;

    // 2. Query SVG sub-elements after dynamic DOM insertion
    ...
    let progress = 0;       // Tracks phase change state (0 = solid ice, 1 = liquid)
    let rotationAngle = 0;  // Rotational matrix angle tracker
    let isRunning = false;

    // Absolute Pin Center Coordinates (ViewBox space)
    ...
    function animateSystem() {
      ...
      // Increment progress state parameters
      progress += 0.0025;
      rotationAngle += 1.8;

      // 1. Rotate stirrer paddle around exact origin
      if (stirrerContainer) {
        stirrerContainer.setAttribute('transform', `rotate(${rotationAngle}, ${absoluteCenterX}, ${absoluteCenterY})`);
      }

      // 2. Rotate fluid motion waves synchronously
      if (motionWaves) {
      ...
      // 3. Fade out ice microstates with slight fluid wobble
      if (iceCubes) {
    ...
    startBtn.addEventListener('click', () => {
      if (!isRunning) {
        // Reset properties if re-running
        if (progress >= 1) {
          progress = 0;
          rotationAngle = 0;
          if (iceCubes) iceCubes.style.opacity = 1;
          if (motionWaves) motionWaves.style.opacity = 0;
          if (stirrerContainer) stirrerContainer.removeAttribute('transform');
          if (motionWaves) motionWaves.setAttribute('transform', 'translate(-69.588642,40.733485)');
          if (iceCubes) iceCubes.setAttribute('transform', 'translate(-18.636745,-67.878565)');
        }
