...
description: "Aula de introdução da primeira lei da termodinâmica, da relação entre energia interna, calor e trabalho. Definição de trabalho adiabático e capacidade térmica."
...
<p class="jt">
  e a equação <a href="#TC7.5" style="color: blue; font-style: italic;">TC7.5</a> é conhecida como <b>1ᵃ. Lei da Termodinâmica</b>. Ela é a lei de <b>conservação de energia</b>, que diz que a energia não pode ser criada nem destruída, apenas convertida uma na outra. Podemos imaginar a energia interna como um grande reservatório, de onde podemos extrair calor ou trabalho ($\Delta U \lt 0$), diminuindo a energia interna do sistema, ou onde depositamos calor e trabalho ($\Delta U \gt 0$), aumentando a energia interna do sistema. Notamos, por fim, que, se a variação de energia interna não de depende do caminho, mas o trabalho sim, então o calor será também uma função de caminho, necessariamente. Para dois caminhos distintos, com mesmos estados inicial e final, o $w$ será diferente, mas $\Delta U$, não. Assim, $q$ varia com o caminho tomado, fazendo com que a variação de energia interna seja a mesma nos dois caminhos.
</p>
...
<style>
  ...
  /* Square container holding your cropped SVG */
  #svg-container {
    width: 100%;
    aspect-ratio: 1 / 1;
    display: flex;
...
<div class="simulation-box">
  ...
  <button class="stir-btn" id="start-btn" disabled>Carregando...</button>
</div>
...
<script>
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Load external W_adiabatic.svg
    try {
      const response = await fetch('W_adiabatic.svg');
      ...
      if (loadedSvg) {
        ...
        // Dynamic auto-zoom: Crops canvas so vessel perfectly fills the square container
        const bbox = loadedSvg.getBBox();
        if (bbox.width > 0 && bbox.height > 0) {
          const maxDim = Math.max(bbox.width, bbox.height);
          const cxBox = bbox.x + bbox.width / 2;
          const cyBox = bbox.y + bbox.height / 2;
          const padding = maxDim * 0.03;
          const size = maxDim + (padding * 2);

          loadedSvg.setAttribute('viewBox', `${cxBox - size/2} ${cyBox - size/2} ${size} ${size}`);
          loadedSvg.setAttribute('preserveAspectRatio', 'xMidYMid meet');
        }
      }
    } catch (error) {
      console.error('Error fetching W_adiabatic.svg:', error);
      startBtn.innerText = "Erro ao carregar W_adiabatic.svg";
      return;
    }

    // Helper: Wrap targeted SVG elements in a clean parent <g> group to isolate Inkscape transforms
    function createWrapperGroup(elementId) {
      let el = document.getElementById(elementId);
      
      if (!el && elementId === 'waves') {
        el = document.getElementById('motion-waves') || document.querySelector('[id*="wave"]');
      }

      if (!el) return { raw: null, wrapper: null };
      
      // Clean hidden/disabled styles left over by Inkscape export
      el.style.display = '';
      ...
      return { raw: el, wrapper: wrapper };
    }

    // Wrap groups
    const blades = createWrapperGroup('blades');
    const waves = createWrapperGroup('waves');
    const ice = createWrapperGroup('ice-cubes');

    // Calculate dynamic pivot center of blades in SVG space
    let cx = 100;
    let cy = 100;
    if (blades.wrapper) {
      const bbox = blades.wrapper.getBBox();
      cx = bbox.x + bbox.width / 2;
      cy = bbox.y + bbox.height / 2;
    }

    // Hide waves initially on load
    if (waves.wrapper) waves.wrapper.style.opacity = '0';

    startBtn.innerText = "Realizar trabalho";
    ...
    function animateSystem() {
      ...
      progress += 0.0025; // Total duration ~6.6 seconds at 60fps

      // 1. Dynamic speed bell curve: Intensity is 0 at start, 1.0 at midpoint (progress = 0.5), and 0 at end
      const intensity = Math.sin(progress * Math.PI);
      
      // Speed scales dynamically with intensity (Max speed ~7.5 deg/frame at midpoint)
      const currentSpeed = intensity * 7.5;
      rotationAngle += currentSpeed;

      // 2. Synchronized Rotation (Blades and Waves move together)
      if (blades.wrapper) {
        blades.wrapper.setAttribute('transform', `rotate(${rotationAngle}, ${cx}, ${cy})`);
      }

      if (waves.wrapper) {
        waves.wrapper.setAttribute('transform', `rotate(${rotationAngle}, ${cx}, ${cy})`);
        // Wave visibility swells in lockstep with stirring speed
        waves.wrapper.style.opacity = (intensity * 0.85).toString();
      }

      // 3. Ice Cubes wobble magnitude peaks at midpoint and fades gracefully
      if (ice.wrapper) {
        const wobbleAmp = 0.65 * intensity; // Peak wobble displacement of 0.65px
        const wobbleX = Math.sin(rotationAngle * 0.035) * wobbleAmp;
        const wobbleY = Math.cos(rotationAngle * 0.04) * wobbleAmp;
        ice.wrapper.setAttribute('transform', `translate(${wobbleX}, ${wobbleY})`);
        ice.wrapper.style.opacity = (1 - progress).toString();
      }
    ...
    startBtn.addEventListener('click', () => {
      if (!isRunning) {
        if (progress >= 1) {
          progress = 0;
          rotationAngle = 0;
          if (ice.wrapper) ice.wrapper.style.opacity = '1';
          if (waves.wrapper) waves.wrapper.style.opacity = '0';
          if (blades.wrapper) blades.wrapper.setAttribute('transform', `rotate(0, ${cx}, ${cy})`);
        }
...
<b>Figura 1.</b> Gelo derrentendo através de trabalho adiabático.
</p>
...
<p class="jt">
  O caminho reversível é especial na físico-química por alguns motivos, como veremos ainda. Um dos motivos é o fato do caminho reversível ser o <b>trabalho máximo</b> que pode ser <b>extraído</b> do sistema, e também o <b>trabalho mínimo</b> que precisa ser realizado <b>sobre</b> o sistema para que uma transformação ocorra.
</p>
...
<div class="toolbox" id="work-tool">
    ...
    <p class="jt">
     O trabalho pode ser calculado, para cada modelo de moléculas interagentes, através das seguintes equações:<br>
     
     Gás ideal: <a href="../A2/#TC2.5" style="color: blue; font-style: italic;">TC2.5</a>
     <div id="LVC7.1" class="mathbox-wrapper">
      <div class="mathbox">
        $$w_{rev}^{GI}= -nRT \ln\left(\frac{V_f}{V_i}\right)$$
      </div>
      <div class="eq-numbers">
        <span style="font-weight: bold; color: blue;">LVC7.1</span>
      </div>  
     </div>
     
     Esfera rígida: <a href="../A2/#TC5.7" style="color: blue; font-style: italic;">TC5.7</a>
     <div id="LVC7.2" class="mathbox-wrapper">
      <div class="mathbox">
        $$w_{rev}^{ER} = -nRT \ln\left(\frac{V_f - nb}{V_i - nb}\right)$$
      </div>
      <div class="eq-numbers">
        <span style="font-weight: bold; color: blue;">LVC7.2</span>
      </div>  
     </div>     

     Potencial do poço quadrado: <a href="../A2/#TC5.9" style="color: blue; font-style: italic;">TC5.9b</a>
     <div id="LVC7.3" class="mathbox-wrapper">
      <div class="mathbox">
        $$\begin{gather}
        \color{blue}{w_{rev}^{PQ} = -nRT \ln\left(\frac{V_f}{V_i}\right) + n^2RTbA \left(\frac{1}{V_f} - \frac{1}{V_i}\right)} \\
        A = 1 + \left(e^{\frac{\epsilon}{kT}} - 1\right)(1 - \lambda^3)
        \end{gather}$$
      </div>
      <div class="eq-numbers">
        <span style="font-weight: bold; color: blue;">LVC7.3</span>
      </div>  
     </div>

     van der Waals: <a href="../A2/#TC5.11" style="color: blue; font-style: italic;">TC5.11c</a>
     <div id="LVC7.4" class="mathbox-wrapper">
      <div class="mathbox">
        $$w_{rev}^{vdW} = -nRT \ln\left(\frac{V_f - nb}{V_i - nb}\right) - n^2a \left(\frac{1}{V_f} - \frac{1}{V_i}\right)$$
      </div>
      <div class="eq-numbers">
        <span style="font-weight: bold; color: blue;">LVC7.4</span>
      </div>  
     </div>
