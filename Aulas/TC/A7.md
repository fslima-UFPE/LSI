...
<style>
  .simulation-box {
    width: 100%;
    max-width: 420px; /* Controls maximum size on desktop */
    margin: 0 auto;
    text-align: center;
    font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;
  }

  /* 1 & 2: Square container that perfectly centers the round vessel */
  #svg-container {
    width: 100%;
    aspect-ratio: 1 / 1; /* Enforces a true square box */
    display: flex;
  ...
  /* Scale SVG to fill the square container */
  #svg-container svg {
    width: 100%;
    height: 100%;
    display: block;
...
<script>
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    // System origin coordinates in SVG space
    const cx = 100.57224;
    const cy = 91.912193;
    ...
    function animateSystem() {
      if (progress >= 1) {
        ...
        // Hide waves on completion
        if (motionWaves) motionWaves.style.opacity = 0;
        return;
      }

      progress += 0.0025;
      rotationAngle += 3.0; // Slightly increased rotation speed for realistic fluid motion

      // 3. Stirrer Rotation around true pin center
      if (stirrerContainer) {
        stirrerContainer.setAttribute('transform', `rotate(${rotationAngle} ${cx} ${cy})`);
      }

      // 4. Wave Motion & Opacity Fix
      if (motionWaves) {
        // Correct transform order: keeps translation offset then rotates around local center
        motionWaves.setAttribute('transform', `translate(-69.588642 40.733485) rotate(${rotationAngle} 170.160882 51.178708)`);
        
        // Rapidly ramps up wave opacity so waves appear immediately during stirring
        const waveOpacity = Math.min(1, progress * 5);
        motionWaves.style.opacity = waveOpacity;
        motionWaves.setAttribute('opacity', waveOpacity);
      }

      // Ice melting fade with micro-wobble
      if (iceCubes) {
