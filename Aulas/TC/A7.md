...
<style>
  .simulation-box {
    width: 100%;
    max-width: 380px; /* Controls container size on screen */
    margin: 0 auto;
    text-align: center;
    font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;
  }

  /* Square container that tightly frames the zoomed vessel */
  #svg-container {
    width: 100%;
    aspect-ratio: 1 / 1;
    display: flex;
  ...
  /* Force SVG to stretch to container boundaries */
  #svg-container svg {
    width: 100% !important;
...
<script>
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
      ...
      // Force view crop to zoom into the vessel circle dynamically
      const svgEl = svgContainer.querySelector('svg');
      if (svgEl) {
        svgEl.removeAttribute('width');
        svgEl.removeAttribute('height');
        svgEl.setAttribute('viewBox', '68.5 60 64 64');
    ...
    // Pin center of fluid vessel/stirrer
    const cx = 100.57224;
    const cy = 91.912193;
    ...
    function animateSystem() {
      ...
      progress += 0.0025;
      rotationAngle += 4.0; // Rotation speed

      // 1. Stirrer Rotation (Universal translate-rotate-translate sequence)
      if (stirrerContainer) {
        stirrerContainer.setAttribute(
          'transform',
          `translate(${cx}, ${cy}) rotate(${rotationAngle}) translate(${-cx}, ${-cy})`
        );
      }

      // 2. Motion Waves Rotation & Visibility
      if (motionWaves) {
        motionWaves.setAttribute(
          'transform',
          `translate(${cx}, ${cy}) rotate(${rotationAngle}) translate(${-cx}, ${-cy}) translate(-69.588642, 40.733485)`
        );
        
        // Rapidly make waves visible while stirring
        const waveOpacity = Math.min(0.85, progress * 4);
        motionWaves.style.opacity = waveOpacity;
      }

      // 3. Ice Melting with subtle fluid wobble
      if (iceCubes) {
        const wobbleX = Math.sin(rotationAngle * 0.08) * 0.6;
        const wobbleY = Math.cos(rotationAngle * 0.1) * 0.6;
        iceCubes.setAttribute(
          'transform',
          `translate(${-18.636745 + wobbleX}, ${-67.878565 + wobbleY})`
        );
    ...
    startBtn.addEventListener('click', () => {
      if (!isRunning) {
        if (progress >= 1) {
          ...
          if (motionWaves) motionWaves.style.opacity = 0;
          if (stirrerContainer) {
            stirrerContainer.setAttribute('transform', `translate(${cx}, ${cy}) rotate(0) translate(${-cx}, ${-cy})`);
          }
        }
