...
<style>
  ...
  /* Force the inserted SVG to fill the container */
  #svg-container svg {
    width: 100% !important;
...
<script>
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    // 1. Fetch your custom W_adiabatic.svg file
    try {
      ...
      // Clean inline sizing to allow responsive CSS scaling
    ...
    // 2. Target elements using your updated IDs
    ...
    // Center pivot coordinates for rotation (Adjust if blades wobble off-center)
    const cx = 100.57224;
    const cy = 91.912193;
    ...
    function animateSystem() {
      ...
      // 2. Animate Waves (Fade in quickly + Rotate synchronously)
      if (waves) {
        waves.setAttribute(
          'transform',
          `translate(${cx}, ${cy}) rotate(${rotationAngle}) translate(${-cx}, ${-cy})`
        ...
        waves.style.opacity = waveOpacity;
      }

      // 3. Fade out Ice Cubes with slight liquid drift
      if (iceCubes) {
    ...
    startBtn.addEventListener('click', () => {
      if (!isRunning) {
        if (progress >= 1) {
          ...
          if (waves) waves.style.opacity = 0;
          if (blades) blades.removeAttribute('transform');
        }
