...
<head>
    ...
    <title>Rotating Blades & Smooth Waves</title>
    <style>
        * {
...
<body>
    ...
    <div class="container">
        ...
        <button id="actionBtn">Start Animation</button>
    </div>
    ...
    <script>
        ...
        let angle = 0;
        let waveOpacity = 0;        // Controls the smooth fade-in transition
        let waveProgress = 0;       // Controls the outward movement of the wave ripples
        let currentFrame = 0;
        let isRunning = false;
        
        const DURATION = 240;       // Total frames the animation runs (approx 4 seconds at 60fps)
        const WAVE_DELAY = 40;      // Frame at which waves start fading in
        ...
        function draw() {
            ...
            // ROTATION: Tying this matrix rotation to 'angle' ensures EVERYTHING 
            // drawn inside this space rotates together (Blades + Waves)
            ctx.rotate(angle);
            ...
            // 3. Draw the Waves (Smooth fade-in & locked rotation)
            if (currentFrame > WAVE_DELAY) {
                // Smoothly ramp up opacity over time instead of popping in instantly
                if (waveOpacity < 1) {
                    waveOpacity += 0.04; 
                }
                ...
                // Draw expanding ripples aligned with the blades
                for (let i = 0; i < bladeCount; i++) {
                    ctx.rotate((Math.PI * 2) / bladeCount);
                    
                    ctx.beginPath();
                    // Creates an arc off the tip of each blade that moves outward
                    let radius = 180 + (waveProgress % 40);
                    ctx.arc(0, 0, radius, -0.2, 0.2); 
                    ctx.stroke();
                }
                
                waveProgress += 1.5; // Controls wave speed outward
            }
        ...
        function startAnimation() {
            // Safety check: Stop any currently running loop before starting a new one
            if (animationId) {
                cancelAnimationFrame(animationId);
            }

            // RESET ALL STATES BACK TO ZERO (Fixes the refresh issue)
            angle = 0;
            waveOpacity = 0;
            waveProgress = 0;
            currentFrame = 0;
        ...
        btn.addEventListener('click', () => {
            // Allows restarting even if it's mid-animation, or if it finished completely
            startAnimation();
        });
    </script>
</body>
