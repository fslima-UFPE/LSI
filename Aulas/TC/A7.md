...
<style>
  ...
  /* Square container holding your SVG */
  #svg-container {
    width: 100%;
    aspect-ratio: 1 / 1;
    display: flex;
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
      ...
      if (loadedSvg) {
        ...
        // Dynamic auto-zoom: Crops canvas so the vessel fills the square container
    ...
    // Helper: Wrap an element in a parent <g> group to protect Inkscape transforms
    ...
    function animateSystem() {
      if (progress >= 1) {
        ...
        // Waves disappear completely when stirring stops
        if (waves.wrapper) waves.wrapper.style.opacity = '0';
        return;
      }
      ...
      if (waves.wrapper) {
        ...
        const waveOpacity = Math.sin(progress * Math.PI) * 0.75;
        waves.wrapper.style.opacity = waveOpacity.toString();
      }

      // 3. Ice Cubes wobble gently in place while melting
      if (ice.wrapper) {
        const wobbleX = Math.sin(rotationAngle * 0.1) * 1.2;
        const wobbleY = Math.cos(rotationAngle * 0.12) * 1.2;
