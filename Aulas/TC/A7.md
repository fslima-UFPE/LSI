  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
      ...
      if (loadedSvg) {
        ...
        // Dynamic auto-zoom: Crops canvas so the bath perfectly fills the square container
    ...
    // Element selection with fallback IDs
    const blades = document.getElementById('blades') || document.getElementById('stirrer') || document.getElementById('stirrer-container');
    const waves = document.getElementById('waves') || document.getElementById('motion-waves');
    const iceCubes = document.getElementById('ice-cubes') || document.getElementById('ice_cubes');
    ...
    // Exact pin center from Inkscape coordinates
    const cx = 100.57224;
    const cy = 91.912193;

    // Inkscape base group translations
    const wavesTx = -69.588642;
    const wavesTy = 40.733485;
    const iceTx = -18.636745;
    const iceTy = -67.878565;
    ...
    function animateSystem() {
      if (progress >= 1) {
        ...
        // Ensure waves are completely gone when work stops
        if (waves) waves.style.opacity = '0';
        return;
      }

      progress += 0.0025;
      rotationAngle += 3.5; // Smooth rotational speed

      // 1. Spin blades in-place around true center
      if (blades) {
        blades.setAttribute('transform', `rotate(${rotationAngle}, ${cx}, ${cy})`);
      }

      // 2. Rotate waves around true center + smooth bell-curve opacity
      if (waves) {
        waves.setAttribute('transform', `rotate(${rotationAngle}, ${cx}, ${cy}) translate(${wavesTx}, ${wavesTy})`);
        // Waves swell smoothly during stirring and fade away completely as stirring stops
        const waveOpacity = Math.sin(progress * Math.PI) * 0.65;
        waves.style.opacity = waveOpacity.toString();
      }

      // 3. Ice cubes wobble while melting
      if (iceCubes) {
        const wobbleX = Math.sin(rotationAngle * 0.08) * 0.8;
        const wobbleY = Math.cos(rotationAngle * 0.1) * 0.8;
        iceCubes.setAttribute('transform', `translate(${iceTx + wobbleX}, ${iceTy + wobbleY})`);
        iceCubes.style.opacity = (1 - progress).toString();
      }
    ...
    startBtn.addEventListener('click', () => {
      if (!isRunning) {
        if (progress >= 1) {
          progress = 0;
          rotationAngle = 0;
          if (iceCubes) iceCubes.style.opacity = '1';
          if (waves) waves.style.opacity = '0';
          if (blades) blades.setAttribute('transform', `rotate(0, ${cx}, ${cy})`);
        }
