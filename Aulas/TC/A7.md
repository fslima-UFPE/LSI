...
<style>
  ...
  /* CRITICAL FIX: Rotates SVG elements around their OWN bounding center */
  #blades, #waves {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
      ...
      if (loadedSvg) {
        ...
        // AUTOMATIC CROP & ZOOM: Fits vessel tightly inside the square container
        const bbox = loadedSvg.getBBox();
        if (bbox.width > 0 && bbox.height > 0) {
          const maxDim = Math.max(bbox.width, bbox.height);
          const cx = bbox.x + bbox.width / 2;
          const cy = bbox.y + bbox.height / 2;
          const padding = maxDim * 0.04; // 4% padding border
          const size = maxDim + (padding * 2);

          loadedSvg.setAttribute('viewBox', `${cx - size/2} ${cy - size/2} ${size} ${size}`);
    ...
    function animateSystem() {
      if (progress >= 1) {
        ...
        // Keep a subtle liquid wave visible after stirring completes
        if (waves) waves.style.opacity = '0.35';
        return;
      }

      progress += 0.003;
      rotationAngle += 5.0; // Spin speed

      // 1. Pure CSS Rotation for Blades (Spins in-place without moving position)
      if (blades) {
        blades.style.transform = `rotate(${rotationAngle}deg)`;
      }

      // 2. Waves spin & stay visible while stirring
      if (waves) {
        waves.style.transform = `rotate(${rotationAngle}deg)`;
        const activeOpacity = Math.min(0.85, progress * 4);
        waves.style.opacity = activeOpacity.toString();
      }

      // 3. Ice Cubes melt in place without jumping/translating
      if (iceCubes) {
