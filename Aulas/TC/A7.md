...
<head>
    ...
    <style>
        /* Restored clean, light background styling */
        body {
            display: flex;
...
<body>
    ...
    <script>
        ...
        // Fix 1 variables: Smooth wave transition tracking
        let waveOpacity = 0;
        let waveRadius = 0;
        ...
        function draw() {
            ...
            // Apply rotation globally to this matrix 
            // (Fix 1: This forces the waves drawn inside to rotate with the blades)
            ctx.rotate(angle);

            // Draw Original Blades
            ctx.fillStyle = '#333333';
            const totalBlades = 3;
            for (let i = 0; i < totalBlades; i++) {
                ctx.rotate((Math.PI * 2) / totalBlades);
                ctx.fillRect(-10, -160, 20, 160);
            }

            // Draw Waves with smooth fade-in
            if (currentFrame > 40) {
                // Fix 1: Incrementally fade opacity in instead of popping up suddenly
                if (waveOpacity < 1) {
                    waveOpacity += 0.04; 
                }

                ctx.strokeStyle = `rgba(0, 122, 255, ${waveOpacity})`;
                ctx.lineWidth = 3;

                // Render waves aligned to each blade's rotation space
                for (let i = 0; i < totalBlades; i++) {
                    ctx.rotate((Math.PI * 2) / totalBlades);
                    ctx.beginPath();
                    // Draws an arc extending just past the tip of the blade
                    ctx.arc(0, 0, 160 + waveRadius, -0.2, 0.2);
                    ctx.stroke();
                }

                // Expand the wave ripples outward
                waveRadius += 1.5;
            }

            ctx.restore();

            // Center Pin (drawn statically on top)
            ctx.beginPath();
        ...
        // Fix 2: Click handler clears previous logic and resets all variables cleanly
        btn.addEventListener('click', () => {
            if (animationId) {
                cancelAnimationFrame(animationId);
            }

            // Resetting timeline states completely so a page refresh isn't needed
            angle = 0;
            currentFrame = 0;
            waveOpacity = 0;
            waveRadius = 0;
