...
description: "Aula de definção de gases ideais, equação virial e equação de estado. Interpretação microscópica da temperatura. Relação de pressão e frequência de colisão."
previous_class: "/LSI/Aulas/TC/A1"
next_class: "/LSI/Aulas/TC/A3"
---
...
<div class="toolbox">
  ...
  <div class="toolbox-content">
      ...
      <div class="jsbox-col-title" style="font-size: 1.2em; border-bottom: 2px solid #eee; padding-bottom: 10px; margin-bottom: 20px; margin-top: 20px; color: #003366;">
        Simulando gases ideais
      </div>

      <p class="jt">
      Podemos imaginar um gás ideal monoatômico bidimensional, isto é, um gás sem estrutura interna que pode se mover apenas no plano <i>xy</i>, ao invés de um espaço tridimensional. Apesar de bidimensional, este gás ainda se comporta idealmente: é puntiforme e não interage com outras moléculas. Vimos que a origem da pressão de um gás ideal é a colisão das moléculas com as paredes do recipiente - único termo avaliado na equação <a href="#TC2.1" style="color: blue; font-style: italic;">TC2.1</a>. A caixa de simulação abaixo permite que você varie o número de partículas, $N$, em uma caixa 2D de aresta $L$ e que fixe a temperatura, $T$, do sistema, bem como a massa , $m$, das partículas. Apesar de puntiformes, as moléculas abaixo possuem um raio, para uma visualização melhor. Contudo, notem que as moléculas não colidem! Também, reparem que as moléculas com menor velocidade possuem coloração azulada, enquanto as moléculas com maior velocidade possuem coloração avermelhada - moléculas com velocidades indumentárias possuem coloração entre as duas. Ao final da simulação, você poderá visualizar o deslocamento das moléculas, assim como a frequência de colisão das partículas com as paredes da caixa 2D. Por fim, note que a ausência de colisões entre moléculas faz com que elas tenham sempre a mesma velocidade ao longo de toda a simulação. Como varia a frequência de colisão com as variáveis acima?
      </p>

      <div class="interactive-area" id="gas-ideal">
        
        <div class="jsbox-control-panel">
          <div class="jsbox-controls-grid" style="display: flex; flex-wrap: wrap; gap: 15px;">
            ...
            <div style="display: flex; align-items: center; justify-content: center; flex: 1 1 150px;">
      ...
      <p class="jt">
            Na seção anterior, é possível visualizar como as mudanças dos valores de temperatura ($T$), aresta da caixa ($L$), massa ($m$) e número das partículas ($N$) afetam a frequência de colisão das moléculas com as paredes da caixa. A relação entre a frequência $f$ e as demais variáveis é conhecida e dada por:
      </p>

      <div id="LVC2.1" class="mathbox-wrapper">
          <div class="mathbox">
              $$\color{blue}{\boldsymbol{f = k_1 \frac{N}{L} \sqrt{\frac{T}{m}}}}$$
          </div>
          <div class="eq-numbers">
            <span style="font-weight: bold; color: blue;">LVC2.1</span>
          </div>
      </div>

          <p class="jt">
            sendo $k_1$ uma constante de proporcionalidade. É possível estabelecer, também, uma relação entre pressão ($p$) e a frequência de colisão. Para um sistema bidimensional, esta relação é:
          </p>

          <div id="LVC2.2" class="mathbox-wrapper">
            <div class="mathbox">
                $$\color{blue}{\boldsymbol{p = \frac{f^2 m}{N} = k_2 \frac{N T}{L^2}}}$$
            </div>
            <div class="eq-numbers">
              <span style="font-weight: bold; color: blue;">LVC2.2</span>
            </div>
          </div>

          <p class="jt">
            Assim como $k_1$, $k_2$ também é uma constante de proporcionalidade, e ambas possuem unidades diferentes. Note que as dependências de $p$ e $f$ com as variáveis $T, N, L$ e $m$ são distintas! É interessante notar que a frequência de colisão para um sistema tridimensional seria proporcional às mesmas variáveis do sistema bidimensional, mas com $k_1$ diferente. Já a pressão, esta é proporcional a $f^2$, a $m$ e inversamente propocional a $N$. Contudo, é também inversamente proporcional a $L$, fazendo com que $ p \propto 1/V$. Claramente, a constante $k_2$ também será diferente para o sistema 3D. Entretanto, ressaltamos aqui a utilidade do modelo bidimensional: a origem da pressão é a mesma do sistema tridimensional. Por fim, notem que a frequência de colisão depende da massa da partícula, mas não a pressão!
          </p>
      ...
      <div class="jsbox-col-title" style="font-size: 1.2em; border-bottom: 2px solid #eee; padding-bottom: 10px; margin-bottom: 20px; color: #003366;">
        Construindo as curvas de frequência e pressão
      </div>

      <p class="jt">
       Para ver na prática o efeito de cada uma destas propriedades na frequência e na pressão, utilize a ferramenta de construção de gráficos abaixo. Nela, é possivel ver a relação entre as variáveis do sistema bidimensional acima, incluindo a frequência de colisão.
      </p>

      <div id="lab-graficos">
        <div class="jsbox-control-panel">
          <div class="jsbox-controls-grid" style="display: flex; flex-wrap: wrap; gap: 20px;">
            ...
            <div style="flex: 1 1 200px;">
              ...
              <div class="jsbox-input-row"><label>N:</label><input type="number" id="base-n" class="jsbox-input" value="150" disabled></div>
              <div class="jsbox-input-row"><label>L:</label><input type="number" id="base-l" class="jsbox-input" value="70"></div>
              <div class="jsbox-input-row"><label>T:</label><input type="number" id="base-t" class="jsbox-input" value="300"></div>
              <div class="jsbox-input-row"><label>m:</label><input type="number" id="base-m" class="jsbox-input" value="50"></div>
            </div>
            ...
            <div style="display: flex; flex-direction: column; justify-content: center; flex: 1 1 200px;">
              <p style="font-size: 0.85em; color: #666; text-align: center; margin-bottom: 15px;">
                Preencha os 5 pontos acima e clique para calcular as equações <a href="#LVC2.1" style="color: blue; font-style: italic;">LVC2.1</a> e <a href="#LVC2.2" style="color: blue; font-style: italic;">LVC2.2</a>.
              </p>
              <button id="btn-plot" class="jsbox-btn jsbox-btn-primary" style="padding: 15px; font-weight: bold; font-size: 1rem; width: 100%;">
                GERAR GRÁFICOS
              </button>
            </div>
        ...
        <div class="jsbox-vis-layout" style="display: flex; flex-wrap: wrap; gap: 20px; margin-top: 20px; width: 100%; box-sizing: border-box;">
...
<div class="toolbox" id="gas-ideal-tool">
    ...
    <div class="jsbox-col-title" style="font-size: 1.2em; border-bottom: 2px solid #eee; padding-bottom: 10px; margin-bottom: 20px; margin-top: 20px; color: #003366;">
      ...
      <p class="jt">
      Este Laboratório Virtual permite visualizar a relação matemática entre as variáveis de estado de um gás ideal, como as apresentadas na <b>Figura 1</b>, de forma interativa. Selecione duas variáveis dentre as quatro possíveis, fixando os valores das outras duas, e gere as curvas de dependência entre as curvas utilizadas.
      </p>
